...
$urun_ilan_sorgu = "SELECT * FROM urun_ilan WHERE kullanici_id='$kullanici_id'";
...
?>
    ...
    <script>
        function showConfirmation(id) {
            if (confirm("Emin misiniz?")) {
                // Onaylandığında formu gönderme
                document.getElementById("ilan_sil").submit();
                window.location.href="ilan_sil.php?id="+id;
            } else {
                // İptal edildiğinde hiçbir işlem yapma
            }
        }
    </script>
    <div class="container-fluid">
    ...
    <div class="product-item bg-light mb-4 ">
            ...
            <div class="text-center py-4">
                ...
                <form id="ilan_sil" method="post" class="d-flex align-items-center justify-content-center mt-2">
                        <a href="ilan_duzenle.php?id='.$row[0].'" class="nav-item nav-link bg-primary text-dark w-50">Düzenle</a>
                        <button type="submit" onclick="showConfirmation('.$row[0].')" class="nav-item nav-link bg-primary text-dark ml-4">Kaldır</button>
                        </form>
            </div>
        </div>
    </a>
